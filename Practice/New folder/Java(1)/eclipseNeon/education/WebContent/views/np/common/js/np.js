    ...
    window.fujitsu.intarfrm.kitnp = kitnp = {
        ...
        */
        renderMessageArea: function (formInfo) {
            var ul, targetElements, targetItemInfo,
                formMessages = formInfo.formUtil.getMessages('html'),
            ...
            $('#' + formInfo.formId + '.input-validation-error').removeClass('.input-validation-error');
            ...
            if (formMessages.length > 0) {
                ...
                if (inputErrors.length > 0) {
                    ul.append($('<li>').append($('<span/>').text(inputErrors.length + "required fields are missing")));
                }
                // 業務エラーの場合は、詳細なメッセージを表示する
                if (businessErrors.length > 0) {
                    $.each(businessErrors, function (i, msg) {
                        ul.append($('<li>').append($('<span/>').html(messages.createText(msg))));
                    });
                }
            ...
            $.each(inputErrors, function (i, msg) {
                if ($.isArray(msg.target) && msg.target.length > 0) {
                    $.each(msg.target, function () {
                        var targetElements = formInfo.formUtil.getElementsByFormItemId(this.formItemId, this.rowIndex),
                        ...
                        if (!$(targetElements).hasClass('np-input-validation-error')) {
                            ...
                            if (targetItemInfo.controlId === 'rdo' || targetItemInfo.controlId === 'chl') {
                                if (!$.mobile) {
                                    // PC・タブレット版の場合
                                    if ($(targetElements).closest('ul.np-vertical').length > 0) {
                                        $(targetElements).closest('ul.np-vertical').after(errorMsg);
                                    } else {
        ...
        */
        changeFunction: function (formInfo) {
            ...
            */
            ...
            /**
            * showMessageに関しては開発キットが提供しているrenderMessageAreaの呼び出しに変更します。
            */
            ...
            */
            formInfo.skeleton.messages.showCodeMessage = function (context) {
                ...
                $.each(formInfo.formUtil.getCodeMessages(formItemId, 'lf'), function (i, info) {
                    $('<div/>').addClass('np-message').append('<span>' + formInfo.skeleton.messages.createText(info) + '</span>').appendTo($(codeMessageAreaSelector));
                });
            };
