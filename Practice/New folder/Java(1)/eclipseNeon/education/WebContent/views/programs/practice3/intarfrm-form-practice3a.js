...
* @name intarfrm-form-practice3a.js
* @author INTARFRM
*/
(function (window) {
    'use strict';
    ...
    var intarfrm = window.fujitsu.intarfrm,
        ...
        programId = 'practice3', formId = 'practice3a',
    ...
    formInfo.actions = actions = {
        ...
        /**
        * フォームアクション[PRACTICE3A_他フォーム]の処理を実行します。
        * @param context フォームアクションコンテキスト
        */
        invokePractice3a_frm: function (context) {
            /* このアクションの実行前の拡張処理はここに記述します。*/
            // ADD START---------------------------------------------------
            // 新規にウィンドウを開く
            var nWindow = window.open('about:blank', 'windowName' + Date.now().toString(), 'width=800, height=600');
            // ADD END-----------------------------------------------------
            // フォームアクション実行
            skeleton.actions.invokeServerAction(context, function (context, data, xhr) {
                ...
                // ADD START---------------------------------------------------
                // 受け取ったURLを新規に開いたウィンドウに設定する
                if (fwData && fwData.url) {
                    nWindow.location.href = fwData.url;
                } else {
