...
* @name intarfrm-form-tyu01002.js
* @author INTARFRM
*/
(function (window) {
    'use strict';
    ...
    var intarfrm = window.fujitsu.intarfrm,
        ...
        programId = 'tyu010we', formId = 'tyu01002',
    ...
    formInfo.actions = actions = {
        ...
        /**
        * フォームアクション[TYU01002_Other_Form_Add]の処理を実行します。
        * @param context フォームアクションコンテキスト
        */
        invokeTyu01002_frm_add: function (context) {
        ...
        /**
        * フォームアクション[TYU01002_Other_Form_Delete]の処理を実行します。
        * @param context フォームアクションコンテキスト
        */
        invokeTyu01002_frm_del: function (context) {
			var cusid = $("#tyu01002-cusid-" + rowIndex ).text();
			$("#modal-delete-customer-body").show();
            /* このアクションの実行前の拡張処理はここに記述します。*/
			$("#modal-delete-customer-body").text("Are you sure you want to delete Customer " + cusid +"?");
			$( "#dialog-confirm" ).dialog({
				...
				buttons: {
					"Yes": function() {
						$( this ).dialog( "close" );
						context.postArgs["rowIndex"]  = rowIndex;
						skeleton.actions.invokeServerAction(context, function (context, data, xhr) {
                			var fwData = data.__frameworkData;
                			/* アクションの実行結果に対する拡張処理はここに記述します。*/
  
             				skeleton.actions.formActionCommonCallBack(context, data, xhr);
						$("#modal-delete-customer-body").hide();
        ...
        /**
        * フォームアクション[TYU01002_Other_Form_Edit]の処理を実行します。
        * @param context フォームアクションコンテキスト
        */
        invokeTyu01002_frm_upd: function (context) {
            /* このアクションの実行前の拡張処理はここに記述します。*/
			context.postArgs["rowIndex"]  = rowIndex;
        ...
        /**
        * フォームアクション[TYU01002_Prev_Group_Cancel]の処理を実行します。
        * @param context フォームアクションコンテキスト
        */
        invokeTyu01002_gbfr_cancel: function (context) {
            /* このアクションの実行前の拡張処理はここに記述します。*/
        ...
        /**
        * フォームアクション[TYU01002_Next_Group_Confirm]の処理を実行します。
        * @param context フォームアクションコンテキスト
        */
        invokeTyu01002_gnxt_def: function (context) {
            /* このアクションの実行前の拡張処理はここに記述します。*/
    ...
    */
    setFormEventHandler = function () {
		...
		$(document).on('click', '.btnDelClass', function () {
	    	// get row index here
			
			rowIndex = $(this).data('action-row');
