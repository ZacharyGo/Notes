...
/**
 * <p>
 * フォーム[Customer Master Maintenance]の業務処理クラス。
 * </p>
 *
...
public final class Tyu01001Facade extends Tyu01001FacadeBase {
	...
	/**
	 * <p>
	 * フォームアクション[TYU01001_DB Update(TYU01001_DBU)]の処理を実行する。
	 * </p>
	 *
	 * アクション: DB更新(DBU)<br>
	 *
	...
	public void doTyu01001_dbu(ModelContext<Tyu01001FormBean> context) throws Exception {
		...
		DbTransaction tran = null;
		StatementInsert stmtM_customer = null;
		...
		String cusID = form.CARD.getCUSID();
		try {
			// verify the customer id if exist
			tran = context.getTransaction();
			IfmConnection conn = tran.getConnection();
			...
			selectBuilder.addColumn(customerEntity.columns());

			if (!StringHelper.isNullOrEmpty(cusID)) {
				selectBuilder.addWhereBindParam(customerEntity.CUSID, SqlOperatorType.EQUAL, cusID);
			}
			...
			int intResult = stmtM_customer.insert();
			if (intResult != 1) {
	...
	/**
	 * <p>
	 * フォームアクション[TYU01001_Back(TYU01001_FRM_BACK)]の処理を実行する。
	 * </p>
	 *
	 * アクション: 他フォーム(FRM)<br>
	 *
	...
	/**
	 * Update Customer DB from form inputs
	 * 
	 * @param context
	 * @throws Exception
	 */
	private void updateCustomerDB(ModelContext<Tyu01001FormBean> context) throws Exception {
		Tyu01001FormBean form = context.getFormBean();
		DbTransaction tran = null;
