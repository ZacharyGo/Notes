...
import com.fujitsu.intarfrm.codes.sotresch.SotreschCodeActions;
import com.fujitsu.intarfrm.helpers.commons.codes.data.CodeMapping;

/**
 * <p>項目[BTNSEARCH]に割り当てたコード定義クラス。</p>
 *
 * 項目名：[ButtonSearch]（標題：Search）<br>
 * コードID：[SOTRESCH]、コード名：[Store Search]（標題：）<br>
 * 
 * @author INTARFRM継承ファイル
 */
public class BtnsearchCodeActionBase
        extends SotreschCodeActions.SotreschCodeAction {
    ...
    public BtnsearchCodeActionBase() {
        this(new SotreschCodeActions());
    }

    /**
     * <p>コンストラクタ。</p>
     */
    public BtnsearchCodeActionBase(SotreschCodeActions actions) {
		actions.super();
    ...
    /**
     * 項目名：[ButtonSearch]（標題：Search）に割り当てたコード：[Store Search] のコードマッピング情報クラス
     */
