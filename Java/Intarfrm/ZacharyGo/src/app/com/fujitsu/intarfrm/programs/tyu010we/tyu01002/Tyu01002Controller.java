...
public class Tyu01002Controller extends Tyu01002ControllerBase {
    ...
    public ActionResult _Load(FormActionContext context) {
        if (context.createNewFormBean()) {
            ...
            bean.CARD.setLOGONDATE(Timestamp.valueOf(LocalDateTime.now()));        // ZSG Code (20211209) - Set Logon Date
            // FormBeanを保存する
    ...
    public ActionResult tyu01002_frm_add(FormActionContext context) {
    	...
    	context.setNextForm(ProgramIds.tyu010we.tyu01001); // Set which form to go next
    	context.getUserMap().put(Tyu010weConstants.ContextMapKey.MODE, Tyu010weConstants.ADD_MODE); // Indicate what mode to select when Customer Master Maintenance
    ...
    public ActionResult tyu01002_frm_upd(FormActionContext context) {
		...
		String customerID = bean.M1.displayRowAt(context.getClicked().getRowNo().intValue()).getCUSID(); // Get Customer ID
		
		if(!customerID.isEmpty()) { 
			context.getUserMap().put(Tyu010weConstants.ContextMapKey.CUSTOMERID, customerID); // Save Customer ID info
	    	context.getUserMap().put(Tyu010weConstants.ContextMapKey.MODE, Tyu010weConstants.EDIT_MODE); // Indicate te mode type when Customer Maintenance is open
		}	
    ...
    public ActionResult tyu01002_frm_del(FormActionContext context) {
    	// ZSG Code Added (20211213) {Begin}
    			...
    			String customerID = bean.M1.displayRowAt(context.getClicked().getRowNo().intValue()).getCUSID(); // Get the Customer ID to be deleted
