...
public final class Tyu01002FormBean extends ProjectFormDataSet<Tyu01002FormBean.Tyu01002FormInfo, Tyu01002CardBean> {
    ...
    public final class Tyu01002G1Info extends FormGroupInfo {
        ...
        public final class Tyu01002G1CardItem extends FormItemConstant {
            ...
            Tyu01002G1CardItem(Tyu01002G1Info owner) {
                ...
                this.SEARCHBTN = super.items().get("SEARCHBTN");
                this.STORENAME = super.items().get("STORENAME");
                this.BTNDEF = super.items().get("BTNDEF");
            }

            /**
             * <p>項目[LogonDate]（標題：LogonDate）の定義情報。</p>
             * 
             * グループ：[G1]、領域：[カード]<br>
             * コントロール：[LBL]（ラベル）<br>
             */
            @ItemBasicInfo(sortNo=100, id="logondate", name="LogonDate", group=FormGroupType.G1, border=FormBorderType.CARD, attribute="D", control=ItemControl.LBL)
            ...
            /**
             * <p>項目[LogonUser]（標題：LogonUser）の定義情報。</p>
             * 
             * グループ：[G1]、領域：[カード]<br>
             * コントロール：[LBL]（ラベル）<br>
             */
            @ItemBasicInfo(sortNo=200, id="logonuser", name="LogonUser", group=FormGroupType.G1, border=FormBorderType.CARD, attribute="SN5", control=ItemControl.LBL)
            ...
            /**
             * <p>項目[StoreID]（標題：StoreID）の定義情報。</p>
             * 
             * グループ：[G1]、領域：[カード]<br>
             * コントロール：[TXT]（１行テキストボックス）<br>
             */
            @ItemBasicInfo(sortNo=400, id="storeid", name="StoreID", group=FormGroupType.G1, border=FormBorderType.CARD, attribute="SB", control=ItemControl.TXT)
            ...
            /**
             * <p>項目[SearchButton]（標題：Search）の定義情報。</p>
             * 
             * グループ：[G1]、領域：[カード]<br>
             * コントロール：[BTN]（ボタン）<br>
             */
            @ItemBasicInfo(sortNo=500, id="searchbtn", name="SearchButton", group=FormGroupType.G1, border=FormBorderType.CARD, attribute="B", control=ItemControl.BTN)
            @ItemBasicCodeInfo(id="STORESCH", code=com.fujitsu.intarfrm.programs.tyu010we.tyu01002.codes.SearchbtnCodeAction.class, codeType=CodeType.COD, autoCodeNameGet = false)
            @ItemBasicCodeCheckInfo(validation=true, overwrite=false)
            @ItemBasicModeInfoAttributes(values = {@ItemBasicModeInfoAttribute(modeType=FormModeType.MODE1, displayMode=ControlDisplayMode.ENABLE, itemCheck=false, requiredCheck=false)})
            public final FormItemInfo SEARCHBTN;

            /**
             * <p>項目[StoreName]（標題：StoreName）の定義情報。</p>
             * 
             * グループ：[G1]、領域：[カード]<br>
             * コントロール：[TXR]（テキストボックスリードオンリー）<br>
             */
            @ItemBasicInfo(sortNo=600, id="storename", name="StoreName", group=FormGroupType.G1, border=FormBorderType.CARD, attribute="SN5", control=ItemControl.TXR)
            @ItemBasicValidationInfo(required=false)
            ...
            /**
             * <p>項目[Confirm Button]（標題：Confirm）の定義情報。</p>
             * 
             * グループ：[G1]、領域：[カード]<br>
             * コントロール：[BTN]（ボタン）<br>
             */
            @ItemBasicInfo(sortNo=700, id="btndef", name="Confirm Button", group=FormGroupType.G1, border=FormBorderType.CARD, attribute="B", control=ItemControl.BTN)
    ...
    public final class Tyu01002G2Info extends FormGroupInfo {
        ...
        public final class Tyu01002G2CardItem extends FormItemConstant {
            ...
            Tyu01002G2CardItem(Tyu01002G2Info owner) {
                ...
                this.CANCELBTN = super.items().get("CANCELBTN");
            }

            /**
             * <p>項目[CancelButton]（標題：Cancel）の定義情報。</p>
             * 
             * グループ：[G2]、領域：[カード]<br>
             * コントロール：[BTN]（ボタン）<br>
             */
            @ItemBasicInfo(sortNo=800, id="cancelbtn", name="CancelButton", group=FormGroupType.G2, border=FormBorderType.CARD, attribute="B", control=ItemControl.BTN)
            @ItemBasicActionInfo(formActionId="TYU01002_GBFR_CANCEL")
            @ItemBasicModeInfoAttributes(values = {@ItemBasicModeInfoAttribute(modeType=FormModeType.MODE1, displayMode=ControlDisplayMode.ENABLE, itemCheck=false, requiredCheck=false)})
            public final FormItemInfo CANCELBTN;
    ...
    public final class Tyu01002M1Info extends FormListInfo {
        ...
        public final class Tyu01002M1Item extends FormItemConstant {
            ...
            /**
             * <p>項目[ListNo.]（標題：ListNo.）の定義情報。</p>
             * 
             * グループ：[G2]、領域：明細[M1]<br>
             * コントロール：[LBL]（ラベル）<br>
             */
            @ItemBasicInfo(sortNo=900, id="meino", name="ListNo.", group=FormGroupType.G2, border=FormBorderType.M1, attribute="NA", control=ItemControl.LBL)
            @ItemBasicDataInfo(total=7, format="11", isSortable=true)
            @ItemBasicModeInfoAttributes(values = {@ItemBasicModeInfoAttribute(modeType=FormModeType.MODE1, displayMode=ControlDisplayMode.ENABLE, itemCheck=false, requiredCheck=false)})
            public final FormItemInfo MEINO;

            /**
             * <p>項目[CustomerID]（標題：CustomerID）の定義情報。</p>
             * 
             * グループ：[G2]、領域：明細[M1]<br>
             * コントロール：[LBL]（ラベル）<br>
             */
            @ItemBasicInfo(sortNo=1000, id="cusid", name="CustomerID", group=FormGroupType.G2, border=FormBorderType.M1, attribute="SB", control=ItemControl.LBL)
            @ItemBasicDataInfo(total=5)
            @ItemBasicModeInfoAttributes(values = {@ItemBasicModeInfoAttribute(modeType=FormModeType.MODE1, displayMode=ControlDisplayMode.ENABLE, itemCheck=false, requiredCheck=false)})
            public final FormItemInfo CUSID;

            /**
             * <p>項目[CustomerName]（標題：CustomerName）の定義情報。</p>
             * 
             * グループ：[G2]、領域：明細[M1]<br>
             * コントロール：[LBL]（ラベル）<br>
             */
            @ItemBasicInfo(sortNo=1100, id="cusname", name="CustomerName", group=FormGroupType.G2, border=FormBorderType.M1, attribute="SN5", control=ItemControl.LBL)
            @ItemBasicDataInfo(total=30)
            @ItemBasicModeInfoAttributes(values = {@ItemBasicModeInfoAttribute(modeType=FormModeType.MODE1, displayMode=ControlDisplayMode.ENABLE, itemCheck=false, requiredCheck=false)})
            ...
            /**
             * <p>項目[TelephoneNo.]（標題：TelephoneNo.）の定義情報。</p>
             * 
             * グループ：[G2]、領域：明細[M1]<br>
             * コントロール：[LBL]（ラベル）<br>
             */
            @ItemBasicInfo(sortNo=1300, id="telno", name="TelephoneNo.", group=FormGroupType.G2, border=FormBorderType.M1, attribute="SC", control=ItemControl.LBL)
            @ItemBasicDataInfo(total=17)
            @ItemBasicModeInfoAttributes(values = {@ItemBasicModeInfoAttribute(modeType=FormModeType.MODE1, displayMode=ControlDisplayMode.ENABLE, itemCheck=false, requiredCheck=false)})
            public final FormItemInfo TELNO;

            /**
             * <p>項目[ボタン変更]（標題：Edit）の定義情報。</p>
             * 
             * グループ：[G2]、領域：明細[M1]<br>
             * コントロール：[BTN]（ボタン）<br>
             */
            @ItemBasicInfo(sortNo=1400, id="btnupd", name="ボタン変更", group=FormGroupType.G2, border=FormBorderType.M1, attribute="B", control=ItemControl.BTN)
            @ItemBasicActionInfo(formActionId="TYU01002_FRM_UPD")
            @ItemBasicModeInfoAttributes(values = {@ItemBasicModeInfoAttribute(modeType=FormModeType.MODE1, displayMode=ControlDisplayMode.ENABLE, itemCheck=false, requiredCheck=false)})
            public final FormItemInfo BTNUPD;

            /**
             * <p>項目[ボタン削除]（標題：Delete）の定義情報。</p>
             * 
             * グループ：[G2]、領域：明細[M1]<br>
             * コントロール：[BTN]（ボタン）<br>
             */
            @ItemBasicInfo(sortNo=1500, id="btndel", name="ボタン削除", group=FormGroupType.G2, border=FormBorderType.M1, attribute="B", control=ItemControl.BTN)
