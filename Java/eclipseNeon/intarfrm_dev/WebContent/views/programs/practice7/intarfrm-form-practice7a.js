...
* @name intarfrm-form-practice7a.js
* @author INTARFRM
*/
(function (window) {
    'use strict';
    ...
    var intarfrm = window.fujitsu.intarfrm,
        ...
        programId = 'practice7', formId = 'practice7a',
    ...
    formInfo.actions = actions = {
        ...
        /**
        * フォームアクション[PRACTICE7A_DBU_REGISTER]の処理を実行します。
        * @param context フォームアクションコンテキスト
        */
        invokePractice7a_dbu_register: function (context) {
            ...
            skeleton.actions.invokeServerAction(context, function (context, data, xhr) {
                ...
                // CUSTOMDATAMAP - START
                // get customDataMap from server
                if (data.userData) {
                    alert(data.userData["customDataMapKey"]);
                }
                // CUSTOMDATAMAP - END

                // CLIENTUSERDATA - START
                alert(formUtil.getClientUserData().clientUserDataKey);
                // CLIENTUSERDATA - END
        ...
        /**
        * フォームアクション[PRACTICE7A_DBU_UPDATE]の処理を実行します。
        * @param context フォームアクションコンテキスト
        */
        invokePractice7a_dbu_update: function (context) {
            /* このアクションの実行前の拡張処理はここに記述します。*/
        ...
        /**
        * フォームアクション[PRACTICE7A_FRM_CANCEL]の処理を実行します。
        * @param context フォームアクションコンテキスト
        */
        invokePractice7a_frm_cancel: function (context) {
            /* このアクションの実行前の拡張処理はここに記述します。*/
