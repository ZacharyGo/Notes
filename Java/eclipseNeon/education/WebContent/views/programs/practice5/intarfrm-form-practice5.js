    ...
    formInfo.actions = actions = {
        ...
        */
        invokePractice5_frm: function (context) {
            ...
            // ADD START---------------------------------------------------
            // idがdialogのdivタグにダイアログに表示するメッセージを作成する。
            $('#dialog').html('Are you sure you want to delete?');

            // idがdialogのdivタグをダイアログとして表示する。
            // オプションに位置を中央、モーダルON、ボタンの配置を設定する。
            // ボタンを押した時の処理をそれぞれに実装する。
            $('#dialog').dialog({
                position: {
                    of: window,
                    at: "center",
                ...
                buttons: {
                    'Yes': function () {
                        // [はい]が選択された場合の処理を実装する。
                        // [はい]が選択された場合は後続の処理を実行するため、
                        // 後続の処理[skeleton.actions.invokeServerAction]を行う。
                    	//処理が完了した後はダイアログをクローズする。
                        $(this).dialog('close');
                    },
                    'No': function () {
                        // [いいえ]が選択された場合の処理を実装する。
                        // [いいえ]が選択された場合は後続の処理を実行しない。
